...
public class MatixxGameTopRequestHandler extends BaseRequestHandler {
    ...
    /**
     * Handles an incoming request for <code>matixx_gametop.get</code>
     * @param requestBody The XML document of the incoming request.
     * @param request The Spark request
     * @param response The Spark response
     * @return A response object for Spark
    ...
    private Object handleGetRequest(final Element requestBody,Request request, Response response) {
        ...
        if(players!=null){
            ...
            LOG.error("Can't load player profile because /matixx_gametop/player not found");
        }
    ...
    private HashMap<Integer, HashMap<Integer, Object[]>> sortScoresByTopScore(final List<matixxStageRecord> records) {
        ...
        // the array: a[0] = map for storing fc and exc flag, a[1] = top UserSongRecord
        ...
        for (matixxStageRecord record : records) {
            if (!topScores.containsKey(record.getMusic().getMusicid())) {
                ...
                if (!topScores.get(record.getMusic().getMusicid()).containsKey(record.getSeq())) {
                    ...
                    if(record.isExcellent()) { temp_map.put("e", true); }
    ...
    /**
     * Get per song max skill record.
     * @param records The HashMap after sorting
     * @return The max skill record for this song
     */
    ...
    /**
     * Get flag map by record.
     * @param record A matixxStageRecord
     * @return The HashMap indicating the fullcombo and excellent status.
     */
    ...
    /**
     * Get flag count.
     * @param seq The difficulty of the record
     * @param flag boolean
     * @return The flag count by difficulty.
     */
